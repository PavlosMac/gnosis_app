...
export default function ShuffledDeck({ numCards, selectedCards, onSelectCard }: ShuffledDeckProps) {
  ...
  return (
    <div ref={deckRef} className="w-full px-0 sm:px-1 flex items-start">
      <div className="flex flex-wrap justify-center gap-x-[2px] gap-y-[1px] mx-auto w-full sm:max-w-[856px]">
        {shuffledDeck.slice(0, 78).map((card, idx) => {
          const selected = selectedCards.find((c) => c.idx === idx);
          return (
            <button
              key={idx}
              className={`relative group ${selected ? 'selected' : ''} overflow-visible transition-[z-index] duration-0 z-0 hover:z-50`}
            ...
            >
              ...
              {!selected && (
                <div className="absolute -inset-1 bg-gradient-to-r from-[#d4af37] via-[#8a2be2] to-[#d4af37]
                                rounded-lg opacity-0 group-hover:opacity-70 group-active:opacity-90 blur transition-all duration-300 pointer-events-none" />
              )}

              {/* Flip container - using proper tarot card aspect ratio (approximately 5:8) */}
              <div className={`relative card-flip-inner ${selected ? 'card-flipped' : ''} transition-transform duration-300 group-hover:scale-115 group-active:scale-95`}
                   style={{ width: 'clamp(40px, 5vw, 60px)', aspectRatio: '5/8' }}>
                {/* Back face (ankh) */}
                <div className="card-flip-face card-flip-back absolute inset-0 bg-gradient-to-br from-[#1a0033] to-[#2d1b4e]
                                rounded-sm border border-[#d4af37]/60 shadow-xl overflow-hidden
                  ...
                  <div className="absolute top-0.5 left-0.5 text-[#d4af37]/40 text-[0.35em]">✦</div>
                  <div className="absolute top-0.5 right-0.5 text-[#d4af37]/40 text-[0.35em]">✦</div>
                  <div className="absolute bottom-0.5 left-0.5 text-[#d4af37]/40 text-[0.35em]">✦</div>
                  <div className="absolute bottom-0.5 right-0.5 text-[#d4af37]/40 text-[0.35em]">✦</div>
                ...
                <div className="card-flip-face card-flip-front absolute inset-0 rounded-sm overflow-hidden border border-[#d4af37] shadow-[0_0_15px_rgba(212,175,55,0.4)] bg-[#f5f5dc]">
                  {selected && (
                    <img
