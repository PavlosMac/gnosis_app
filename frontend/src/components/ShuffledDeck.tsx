      ...
      <div className="flex flex-wrap justify-center gap-x-[2px] gap-y-[2px] sm:gap-y-[1px] mx-auto w-full max-w-[400px] sm:max-w-[856px]" style={{ perspective: '600px' }}>
        {shuffledDeck.slice(0, 78).map((card, idx) => {
          ...
          return (
            <button
              key={idx}
              className={`relative group ${selected ? 'selected' : ''} overflow-visible z-0 active:z-50 sm:hover:z-50 transition-[z-index] duration-0`}
            ...
            >
              ...
              <div className={`relative card-flip-container card-flip-inner ${selected ? 'card-flipped' : ''} transition-transform duration-300 sm:group-hover:scale-115 group-active:scale-95`}>
                {/* Back face (ankh) */}
                <div className="card-flip-face card-flip-back absolute inset-0 bg-gradient-to-br from-[#1a0033] to-[#2d1b4e]
                  ...
                  <div className="absolute top-0.5 left-0.5 text-[#d4af37]/40 text-[0.25em] sm:text-[0.35em]">✦</div>
                  <div className="absolute top-0.5 right-0.5 text-[#d4af37]/40 text-[0.25em] sm:text-[0.35em]">✦</div>
                  <div className="absolute bottom-0.5 left-0.5 text-[#d4af37]/40 text-[0.25em] sm:text-[0.35em]">✦</div>
                  <div className="absolute bottom-0.5 right-0.5 text-[#d4af37]/40 text-[0.25em] sm:text-[0.35em]">✦</div>

                  <div className="absolute inset-0 bg-gradient-to-br from-transparent via-[#d4af37]/10 to-transparent
                                  opacity-0 sm:group-hover:opacity-100 group-active:opacity-100 transition-opacity duration-500" />
                </div>

                {/* Front face (card image) */}
                <div className="card-flip-face card-flip-front absolute inset-0 rounded-sm overflow-hidden border border-[#d4af37] shadow-[0_0_10px_rgba(212,175,55,0.4)] sm:shadow-[0_0_15px_rgba(212,175,55,0.4)] bg-[#f5f5dc]">
                  {selected && (
                    <img
